...
- 总结
  C++通过*构造函数*与*转换函数*实现了其他类型与类类型之间的转换，但是存在一个问题————这些转换都会自动隐式进行(C++推荐最好不要使用隐式转换，建议使用explicit关键字)

##  转换函数与友元函数
在类重载加法运算时，提到过可以使用类成员函数同时也能使用友元函数。但是如果要实现完整的加法运算可能两者都需要使用。**此处应该特别小心二义性问题**
